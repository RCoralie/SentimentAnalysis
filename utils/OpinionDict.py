...
def extractOpinionWords():
    ...
    with codecs.open(Config.OPINION_FILE,'r','utf-8') as f:
...
def saveDicIntoFile(opinionDic):
    ...
    np.save(Config.OPINION_DICT_FILE, np.asarray([opinionDic]))
...
def loadDicFromFile():
    ...
    if not isfile(Config.OPINION_DICT_FILE):
        dic = extractOpinionWords()
        saveDicIntoFile(dic)
        return dic
    
    return np.load(Config.OPINION_DICT_FILE)[0]
